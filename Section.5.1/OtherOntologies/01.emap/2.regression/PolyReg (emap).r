...
myData = readWorksheet(wk, sheet="emap")
...
# plot(RAM_KB, AxiomCount, col=c("orange","blue","green")[Grp], main="EMAP on ELK (Poly.Reg.)")

plot(myData$RAM_KB, myData$AxiomCount, xlab="RAM (KB.)", ylab="Axiom Count", main="EMAP on ELK (Poly.Reg.)", pch=2, cex.main=1.5, frame.plot=FALSE , col=ifelse(myData$Grp==1, "orange", (ifelse(myData$Grp==2, "blue", "green"))))

# and, let's add a legend to clarify the lines
legend(10000, 40000, pch=c(2,2), legend = c("linear model","model2: poly(degree = 2)", "model3: poly(degree = 3)"), 
