...
wk = loadWorkbook("EmapGoFmaChebi.xlsx") 
myData = readWorksheet(wk, sheet="chebi")
...
plot(myData$RAM_KB, myData$AxiomCount, xlab="RAM (KB.)", ylab="Axiom Count", main="CHEBI on ELK (Poly.Reg.)", pch=2, cex.main=1.5, frame.plot=FALSE , col=ifelse(myData$Grp==1, "orange", (ifelse(myData$Grp==2, "blue", "green"))))

# and, let's add a legend to clarify the lines
legend(10000, 350000, pch=c(2,2), legend = c("linear model","model2: poly(degree = 2)", "model3: poly(degree = 3)"), 
