...
public class MainActivity extends Activity {
	...
	public void run(View view) {
		...
		// *** ISA: I hope this technique really collects garbage!
		myGC();
		...
		final Thread reasonerThread = new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					// System.out.println("RAM Beginning:" + usedMemoryBefore +
					// ": KB. RAM End:" + usedMemoryAfter + ": KB. RAM Diff:" +
					...
					try {
						...
						// stop();// Isa: Here stops the timer logging available
						// RAM consumption.
	...
	private void initReasoner() {
		...
		AxiomLoader loader = new Owl2StreamLoader(parserFactory, this.getResources().openRawResource(ontologyId));
...
*/
	...
	*/
	public void runViaTrOWL(View view) {

		File folder = new File(glbFolderOntologies);
		File[] listOfFiles = folder.listFiles();
		for (int i = 0; i < listOfFiles.length; i++) {
			if (listOfFiles[i].isFile()) {
				System.out.println("File sent to TrOWL4Android: " + listOfFiles[i].getName().toString());
				glbOntoFileName = listOfFiles[i].getName().toString();
				executeClassificationTrOWL(glbFolderOntologies + File.separator + glbOntoFileName);
			}
		}

	}
	
	public void executeClassificationTrOWL(String pOntology) {
		...
		try {
			String dOntology = "file://" + pOntology;
			System.out.println("Ontology File :" + dOntology);

			glbStringOfNow = StringOfNow();

			writeText("", glbStringOfNow + "_" + glbOntoFileName, "--- Ontology Processing by TrOWL4Android ---\n");
			writeText("", glbStringOfNow + "_" + glbOntoFileName, "Ontology File:" + glbOntoFileName + "\n");
			writeText("", glbStringOfNow + "_" + glbOntoFileName, "Execution Started:" + glbStringOfNow + "\n");
			...
			File dFile = new File(pOntology);
			ont = om.loadOntologyFromOntologyDocument(dFile);
